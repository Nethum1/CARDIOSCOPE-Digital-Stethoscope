# 🩺 Digital Stethoscope — ESP32 + Local ML + Real-Time Dashboard
...
<details>
  ...
  <ol>
    ...
    <li>
      <a href="#esp32-firmware">ESP32 Firmware</a>
      <ul>
    ...
    <li><a href="#local-server">Local Prediction Server</a></li>
...
<ul>
  ...
  <li>
    <a href="https://docs.espressif.com/projects/esp-idf/en/latest/esp32/" target="_blank">
      <img src="https://cdn.shopify.com/s/files/1/0609/6011/2892/files/doc-esp32-pinout-reference-wroom-devkit.png?width=200"
      alt="ESP32" width="80" height="50">
    </a>&nbsp; ESP32 (PlatformIO + Arduino Framework)
  </li>
  <li>
...
- Capture heart and lung sounds using an **INMP441 I2S digital microphone** attached to an acoustic stethoscope chest piece
- Transmit raw 16kHz audio wirelessly from ESP32 to a **local PC Flask server** over Wi-Fi
- Extract meaningful audio features (**MFCC, Chroma, Spectral Centroid, ZCR, RMS**) using librosa
- Classify sounds as **Normal, Murmur, Arrhythmia, or Extra Sound** using a Random Forest model
- Display live **waveform, MFCC heatmap, probability charts** on a real-time web dashboard
- Allow **patient record management** and **PDF report generation** from the dashboard
...
<img src="https://cdn.shopify.com/s/files/1/0609/6011/2892/files/doc-esp32-pinout-reference-wroom-devkit.png?width=1384"
  alt="ESP32 Pinout" width="700" height="500">
...
The project uses **PlatformIO** in Visual Studio Code — not Arduino IDE. This gives cleaner project structure, automatic library downloads, and better code management.

**Project Structure:**
```
digital-stethoscope-esp32/
├── platformio.ini        ← Board config + library dependencies
├── src/
│   └── main.cpp          ← Main firmware (NOT .ino)
├── include/
│   └── config.h          ← Wi-Fi credentials, pin definitions
└── lib/
```
...
**Firmware Flow:**
1. ESP32 boots → connects to Wi-Fi → green LED on
2. Every 10 seconds (or on button press) → red LED on → records 3s audio via I2S
3. HTTP POST raw bytes to `http://<PC_IP>:5000/predict`
4. Receives JSON response → prints prediction + confidence to Serial Monitor
5. Red LED off → waits for next trigger
...
The Flask server (`local_prediction_server.py`) runs on your PC and handles the full prediction pipeline:

**Endpoints:**

| Endpoint | Method | Description |
|---|---|---|
| `/predict` | POST | Receives raw audio bytes from ESP32, returns prediction + waveform + MFCC |
| `/latest` | GET | Returns the most recent full prediction (dashboard polls this) |
| `/history` | GET | Returns last 50 compact predictions |
| `/status` | GET | Server health + model status |
| `/test` | GET | Inject a synthetic test signal — for debugging without ESP32 |
| `/` | GET | Serves the dashboard HTML |

**Key design decisions:**
- `/predict` returns the **full waveform array (200 points)** and **full MFCC matrix (40×60)** so the dashboard plots real data, not simulated data
- `/latest` is polled every 2 seconds and only triggers a UI update when `timestamp` changes — efficient and responsive
- Server runs with `threaded=True` so multiple concurrent requests don't block
- If no trained model is found, server runs in **demo mode** with realistic fake predictions
...
1. [Your Name](https://github.com/yourusername)

<hr>

<p align="center">
  Made with ❤️ using ESP32 · Python · Flask · librosa · scikit-learn
</p>
